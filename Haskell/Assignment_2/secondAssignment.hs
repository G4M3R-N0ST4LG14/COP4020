...
isPrime x
    ...
    | x `mod` 2 == 0 = False -- Even numbers are not prime numbers
    | otherwise = null [y | y <- [3,5..floor (sqrt (fromIntegral x))], x `mod` y ==0] -- Checks to see if any odd numbers up to the square root of x can divide the number to see if it's prime

-- Main function to generate the list of every other prime
primes = filter isPrime [2..] -- Create an infinite list of primes
problem1 :: Int -> [Int]
problem1 n = take n [x | (x, y) <- zip primes [0..], even y] -- Put primes in an index where the even indices starting at 0 get added to the final list and the odds get filtered


-- 2. Create a function named problem2. Use recursion to solve this problem. This function will take a single integer parameter. It indicates the maximum value of Fibonacci numbers that will be in the list. The final solution will be a list of Fibonacci numbers that have a three as their right-most digit and are less than or equal to n.
problem2 :: Int -> [Int]
generateFibs a b = a : generateFibs b (a + b) -- Creates Fibonacci sequence with recursion by adding a and b together and creating an infinite list where b is the next starting number in the addition
...
hasExactlyThreeFactors x = isPerfectSquare x && isPrime (floor (sqrt (fromIntegral x))) -- If the number x has a whole square root and the square root is a prime number then it is guaranteed to have only 3 factors (1, sqrt(x), and x)
